...
def max_dot_product(first_sequence, second_sequence):
    ...
    assert all(0 <= f <= 10 ** 5 for f in first_sequence)
    assert all(0 <= s <= 10 ** 5 for s in second_sequence)
    sorted_prices = sorted(first_sequence, reverse=True)
    sorted_clicks = sorted(second_sequence, reverse=True)
    max_product = sum([sorted_clicks[i] * sorted_prices[i] for i in range(len(sorted_clicks))])
    return max_product
