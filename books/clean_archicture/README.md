...
Несмотря на то, что программное обеспечение не изнашиваеся, оно моет быть разрушено неуправляемыми зависимостями от
микропрограм (firmware) и оборудования.

Прекратите писать так много микропрограмм и дайте своему коду шанс служить долго. 

Чистая встраиваемя архитектура - архитектура поддерживающая тестирование.

Слой абстракции операционной системы:
- Программное обеспечение
- OSAL (чистая встраиваемая архитектура изолирует ПО от ОС, реализуя Operation System Abstract Layer)
- ОС
- HAL (Hardware Abstraction Layer)
- Микропрограмма
- Оборудование

Слой OSAL может помочь создать точки для тестирования, чтоб можно было тестировать без целевого оборудования и ОС.

Не загромождайте интерфейсные заголочные файлы структурами данных, константами и определения типов, которые нужны только
для реализации. Это приводит к нежелательным зависимостям.

## База данных - это деталь

Структура, которую вы придаете данным в своей приложении, очень важна для архитектуры системы. Но база данных - это не
модель данных. База данных - утилита, обеспечивающая доступ к данным, низкоуровневая деталь - механизм.

## Веб - это деталь

UI - это деталь. Web - это UI, т.е. Web - это деталь. Должны размещать деталь за границами, отделяющих их от основной
бизнес-логики.

## Фреймворк - это деталь

Фреймворк может помочь с реализацией первых возможностей приложения. Однако, достигнув некоторого уровня зрелости, ваш
продукт перерастет фреймворк.

Фреймворк может развиваться в направлении, бесполезном для вас. Бесполезные для вас обновления, а нужные особоенности
исчезают или изменяются так, что вам трудно поспевать за ними.

Не позволяйте фреймворкам проникать в ваш основной код. Внедряйте их в компоненты, которые лишь подключатся
...
Горизонатальные уровни используются как способ группировки по подобию. Уровни должны зависить только от следующего
смежного уровня.

### Упаковка по особеностям

Разделение по вертикали, основанное на объединении связанных особенностей, предметных понятий и общих корней.

### Порты и адаптеры

База кода часто состоит из двух областей: "внутренняя" (предметная) и "внешняя" (инфраструктура). Как луковица.

"Внутренняя" область включается все предметные понятия, а "внешняя" отвечает за взаимодействия с внешним миром (т.е.
GUI, БД, механизм интеграции со сторонними продуктами). Главная правило: "внешняя" область зависит от "внутренней".

### Упаковка по компонентам

Этот подход связывает "бизнес-логику" и код для работы с хранилищем в единое нечто, что мы называем "компонентом".
Компонент - группа функциональных возможностей, находящихся за общим чистым интерфейсом, которые постоянно находятся
внутри среды выполнения, такой как приложение.

Ключемое преимущество подхода - в размещении всего кода, например, имеющего отношение к обратке заказов, в одном месте -
в компоненте OrdersComponent. Задачи внутри компонента все еще разделены - т.е. бизнес-логика отдельно, функции доступа
к БД отдельно.

### Организация и инкапсуляция

Если злоупотреблять public, то это приведение в жизнь архитектурных принципов полагаясь на самодисциплину и инструменты,
выполняющиеся после компиляции, а не опираясь на компилятор.

### Дьявол кроется в деталях реализации

Любые, самые лучшие дизайнерские намерения можн уничтожить в мгновения ока, если не учитывать тонкости стратегии реализации.

Подумайте:
- как ваш дизайн должен отображаться в структуру кода
- как организовать этот код
- какие режимы разделения приенять во время выполнения и компиляции

Старайтесь оставлять открытыми любые возможности, но будьте прагматичными и учитывайте:
- численность вашей команды
- навыки ее членов
- сложность решения
- ограничение по времени и бюджету

Также подумайте, как использовать компилятор для принудительного соблюдения выбранного архитектурного стиля, и следите
за связями в других обрастях, как модели данных.
