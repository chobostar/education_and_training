...
- bpf() — центральный низкоуровневый интерфейс, но не то, с чем обычно работают напрямую. На практике ты пользуешься libbpf, BCC, Go/Rust‑обёртками и bpftool, которые прямо мапятся на команды bpf() и скрывают детали union bpf_attr и сетапа perf/ring буферов.
- Понимание команд bpf() критично для дебага и introspection. Умение читать strace -e bpf,... позволяет проверить, что программы и карты реально создаются/обновляются, как ты ожидаешь, правильно ли выбран тип карты, есть ли BTF и почему верификатор/ядро отвергли вызов.
fd, pinning и ссылки определяют «жизненный цикл» BPF‑объектов. Если не создать дополнительную ссылку (pin или BPF link), программа/карта исчезнут вместе с процессом‑лоадером; наоборот, «забытые» pin/link легко приводят к «висящим» объектам в ядре.
- Разные типы программ — разные пути прикрепления. kprobe/tracepoint используют perf_event_open и ioctl, raw tracepoints и cgroup‑программы — соответствующие команды bpf(), сетевые/XDP — свои механизмы; это нужно учитывать при чтении трейсинга и при написании собственных лоадеров.
- Perf vs ring buffer — важное эволюционное изменение. Ring buffer упрощает модель (один fd для всех CPU) и даёт лучшие гарантии порядка и производительности, поэтому новые приложения стоит строить на BPF_RINGBUF, оставляя perf buffer в основном для обратной совместимости
